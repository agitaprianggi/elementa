...
class ShopProductInfo extends Model
{
    ...
    public $table = SC_DB_PREFIX.'shop_product_info';
    protected $guarded = [];
    ...
    protected static function boot()
    {
        ...
        static::deleting(
            function ($product) {
                ...
                ->where(SC_DB_PREFIX.'shop_custom_field.type', 'shop_product_info')
                ->delete();
            }
        );
        ...
        static::creating(function ($model) {
            if (empty($model->{$model->getKeyName()})) {
                $model->{$model->getKeyName()} = sc_generate_id($type = 'shop_product_info');
            }
        });
