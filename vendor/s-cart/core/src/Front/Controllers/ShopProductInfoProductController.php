...
class ShopProductInfoProductController extends RootFrontController
{
    ...
    private function _productDetail($alias)
    {
        ...
        $product = (new ShopProductInfo)->getDetail($alias, $type = 'alias', $storeId);
        if ($product && $product->status && (!sc_config('product_stock', $storeId) || sc_config('product_display_out_of_stock', $storeId) || $product->stock > 0)) {
            ...
            $productRelation = (new ShopProductInfo)
                ->getProductToCategory($arrCategoriId)
                ->setLimit(sc_config('product_relation', $storeId))
                ->setRandom()
                ->getData();

            sc_check_view($this->templatePath . '.screen.product_info_detail');
            return view(
                $this->templatePath . '.screen.product_info_detail',
                array(
                    'title'           => $product->name,
