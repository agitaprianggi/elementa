...
class ShopCategoryController extends RootFrontController
{
     ...
     * @return void
     */
    public function allCategoriesProcessFrontInfo(...$params)
    {
        if (config('app.seoLang')) {
            $lang = $params[0] ?? '';
            sc_lang_switch($lang);
        }
        return $this->_allCategoriesInfo();
    }
     ...
     * @return [view]
     */
    private function _allCategoriesInfo()
    {
        $sortBy = 'sort';
        $sortOrder = 'asc';
        $filter_sort = sc_request('filter_sort','','string');
     ...
     * @return void
     */
    public function categoryDetailProcessFrontInfo(...$params)
    {
        if (config('app.seoLang')) {
            $lang = $params[0] ?? '';
            $alias = $params[1] ?? '';
        ...
        return $this->_categoryDetailInfo($alias);
    }
     ...
     * @return [view]
     */
    private function _categoryDetailInfo($alias)
    {
        $sortBy = 'sort';
        $sortOrder = 'asc';
        $arrBrandId = [];
        ...
        if ($category) {
            ...
            sc_check_view($this->templatePath . '.screen.product_info_list');
            return view(
                $this->templatePath . '.screen.product_info_list',
                array(
                    'title'       => $category->title,
                    'categoryId'  => $category->id,
                    'description' => $category->description,
                    ...
                    'filter_sort' => $filter_sort,
                    'breadcrumbs' => [
                        ['url'    => sc_route('category.allInfo'), 'title' => sc_language_render('front.categories')],
                        ['url'    => '', 'title' => $category->title],
                    ],
                )
            );
